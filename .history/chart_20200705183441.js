function draw(pos, chart) {
    var el = document.getElementById(pos).getContext('2d')
    console.log(el);
...
console.log(chartData.chart1
    );


    for (let chart in chartData) {
        var  html = '<canvas  draggable="true"  class="list-item myChart" id="chart-'+chartData[chart].id+'"></canvas>';
      var canvas = document.createElement('canvas');
      canvas.className = "list-item";
      canvas.setAttribute('draggable','true');
      canvas.setAttribute('data-chart', JSON.stringify(chartData[chart]));
      console.log(chartData[chart]);
      
      canvas.id = 'chart-'+chartData[chart].id;
        chartWrapper.appendChild(canvas);
        console.log(chartWrapper);
        console.log(document.getElementById('chart-'+chartData[chart].id));
        
        var  el = document.getElementById('chart-'+chartData[chart].id).getContext('2d');
        draw('chart-'+chartData[chart].id,chartData[chart] )
      }
